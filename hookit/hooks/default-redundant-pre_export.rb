...
payload[:generation][:members].each do |member|
  ...
  if member[:type] == 'default'
    ...
    new_member_stat = `ssh -o StrictHostKeyChecking=no #{member[:local_ip]} stat -f --format=\\\"%a %S\\\" /data/var/db/mongodb`
    ...
    execute "send bulk data to new member" do
      command "tar -cf - /data/var/db/mongodb | ssh -o StrictHostKeyChecking=no #{member[:local_ip]} tar -xpf -"
    end
